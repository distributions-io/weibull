...
function pdf( x, options ) {
	...
	if ( isNumber( x ) ) {
		return pdf1( x, this.lambda, this.k );
	}
	...
	if ( isMatrixLike( x ) ) {
		...
		return pdf5( out, x, this.lambda, this.k );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, this.lambda, this.k );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, this.lambda, this.k, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, this.lambda, this.k, opts.accessor );
		}
		return pdf2( out, x, this.lambda, this.k );
	}
	return NaN;
} // end FUNCTION pdf()
