...
function mgf( x, options ) {
	...
	if ( isNumber( x ) ) {
		return mgf1( x, opts.lambda, opts.k );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return mgf5( out, x, opts.lambda, opts.k );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return mgf6( out, x, opts.lambda, opts.k );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return mgf4( x, opts.lambda, opts.k, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return mgf3( out, x, opts.lambda, opts.k, opts.accessor );
		}
		return mgf2( out, x, opts.lambda, opts.k );
	}
	return NaN;
} // end FUNCTION mgf()
